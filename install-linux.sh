...
cat > "$SCRIPT_DIR/Calendar Export.desktop" << EOF
[Desktop Entry]
Version=1.0
Type=Application
...
chmod +x "$SCRIPT_DIR/Calendar Export.desktop"
echo "Created: $SCRIPT_DIR/Calendar Export.desktop"
...
if [[ "${add_to_menu,,}" == "y" ]]; then
    ...
    cp "$SCRIPT_DIR/Calendar Export.desktop" "$HOME/.local/share/applications/calendar-export.desktop"
    update-desktop-database "$HOME/.local/share/applications" 2>/dev/null || true
    echo "Added to application menu."
fi

echo ""
echo "Done. To launch the app, double-click 'Calendar Export.desktop'."
echo "If your file manager asks, choose 'Run' or 'Allow Launching'."
echo ""
echo "To trust it automatically on GNOME, run:"
echo "  gio set \"$SCRIPT_DIR/Calendar Export.desktop\" metadata::trusted true"
